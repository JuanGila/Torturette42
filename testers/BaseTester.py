...
class BaseTester:
	...
	def show_summary(self, norm: str, missing, errors, to_execute):
		<3
		def get_norm_errors():
			def get_fname(line): return norm_func_regex.match(line).group(1)
			def is_file(line): return norm_func_regex.match(line)
			return [get_fname(line) for line in norm.splitlines() if is_file(line)]
		norm_errors = get_norm_errors()
		error_funcs = set()
		for results in errors: error_funcs = error_funcs.union(results[1])
		#
